...
{
    nrf_address_t default_address = {NRF_DEFAULT_ADDRESS};

    memcpy (node->rx.address.bytes, default_address.bytes, 
            sizeof (nrf_address_t));
    memcpy (node->tx.address.bytes, default_address.bytes,
            sizeof (nrf_address_t));
    node->rx.channel = NRF_BROADCAST_CHANNEL;
    node->tx.channel = NRF_BROADCAST_CHANNEL;
    node->rx.id = 0;
    node->tx.id = NRF_MASTER_ID;
}
