...
/** Set the control bits, I, F, T, and M[4:0] of the CPSR.
    @param CPSR control bit mask.  */
__inline __attribute__ ((always_inline)) 
...
/** Set the mode bits M[4:0] of the CPSR.
...
/** Remap SRAM that SRAM exists at 0x200000 as well as at 0x000000.  On
    ...
    causes problems if running from the debugger since theCPU is not
    reset.  Thus we don't toggle the remap bit if the SRAM is already
    remapped.  */
__inline void cpu_sram_remap (void)
{
    uint32_t tmp;
    uint32_t *p;
